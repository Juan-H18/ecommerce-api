...
exports.AgregarOActualizar = async (req, res) => {
    ...
    let item = await CarroItem.findOne({ where: { Id_Usuario: req.user.id, Id_Producto }});
    if (item) {
        item.Cantidad = Cantidad;
        await item.save();
    } else {
        item = await CarroItem.create({ Id_Usuario: req.user.id, Id_Producto, Cantidad });
    }
    res.json(item);
};
...
exports.Eliminar = async (req, res) => {
    ...
    await CarroItem.destroy({ where: { Id_Usuario: req.user.id, Id_Producto }});
